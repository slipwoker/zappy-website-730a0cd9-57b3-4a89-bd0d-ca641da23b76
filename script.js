    ...
    contactForm.addEventListener('submit', function(e) {
      ...
      for (let i = 0; i < formElements.length; i++) {
        ...
        if (field.type === 'tel' && field.value.trim()) {
          ...
          if (!phonePattern.test(field.value.trim()) || field.value.trim().length < 10) {
            isValid = false;
            field.classList.add('error');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "DK accounting & taxes משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '730a0cd9-57b3-4a89-bd0d-ca641da23b76',
                    name: data.name || '',
                    email: data.email || '',
